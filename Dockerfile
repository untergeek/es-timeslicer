...
# Build (or rather Freeze) your code
RUN python3 setup.py build_exe

# This will add the cacert.pem from certifi to the default location will look
# and also move 'build/exe.{system().lower()}-{machine()}-{MAJOR}.{MINOR}' to frozen_build
...
# The path `frozen_build` is from `builder` and `post4docker.py`
COPY --from=builder frozen_build /estimeslicer/
RUN mkdir /.estimeslicer
# This is for the Docker default filepath override
RUN mkdir /fileoutput

USER nobody:nobody
ENV LD_LIBRARY_PATH /estimeslicer/lib:$LD_LIBRARY_PATH
ENTRYPOINT ["/estimeslicer/es-timeslicer"]
